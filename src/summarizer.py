...
3. **格式严格**：
   ...
   - 不要把一段话拆成很多个用bullet point组成的短句子列表，而要尽可能用完整的段落来组织老师的讲解。
   - 这并不是说不要用任何的枚举或者列表。你需要合理使用加粗、列表、表格、分级标题、段首小标题等形式来组织信息，确保结构清晰。当讲话人的逻辑的确符合并列或递进逻辑时，可以使用列表。
   - 出于节省空间考虑，不要使用连续的回车换行，不要出现空行。
4. **公式规范**：所有数学公式或科学变量必须使用规范的 LaTeX 语法（行内公式用 $...$，行间公式用 $$...$$）。由于图床限制，latex公式中不要出现中文。
5. **忠于原文与详略得当**：总结必须详略得当，长度适宜（例如，对于90分钟长度的课程，总结长度应为4000字左右；135分钟的课程，应为6000字左右。输出和输入的长度压缩比例在1:8左右为宜），
...
8. **输入材料格式**：
   ...
   - 无论哪种格式，**PPT 文字是真实的板书 / 课件内容**，比音频识别噪声更可靠；遇到音频识别错的术语（同音字、专有名词），优先以 PPT 文字为准修正。
    ...
    def _call_llm(self, client: OpenAI, model: str,
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"以下是课程《{title}》的录音文本，根据长度，你应该输出的字符数大约为{len(content)/8}字，请开始总结：\n\n{content}",
                },
            ],
            temperature=0.3,
            timeout=180,
